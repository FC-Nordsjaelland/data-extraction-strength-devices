...
st.set_page_config(
    page_title="Strength Data Visualization", page_icon="☀️", layout="wide"
)
st.sidebar.markdown("## Strength Data Visualization")

st.sidebar.header("**Functionality**")
st.sidebar.markdown(
    "- Percent margin: +/- value for +/- 2.5%, fx. for Percent margin = 0.1, the threshold will be set from 2.4% to 2.6%"
)
st.sidebar.markdown(
    "- Increase the margin if the onset/offset are further away from the curve of interest than they should be"
)
st.sidebar.markdown(
    "- Number of splits: intervals/milisecond splits, used for the output file, fx. splits=10 means that we will take only every 10th millisecond into consideration"
)
st.sidebar.markdown(
    "- Zoom: Yes - view the zoomed part of the graphs to assess the peak and onset/offset calculations in more detail"
)
...
def output_calculations(
    path, perc_margin=0.1, splits=10, viz=False, zoom=False, output=False
):
    ...
    df = pd.read_csv(path, delimiter=";", skiprows=6)
    if len(df.columns) == 4:
        df.drop(df.columns[3], inplace=True, axis=1)
    dct_replace = {
        df.columns[0]: "SampleTime[s]",
        df.columns[1]: "Force_left[N]",
        df.columns[2]: "Force_right[N]",
    }
    ...
    acceptable_onset_value_lst_left = [
        float("%.2f" % elem) for elem in acceptable_onset_value_lst_left
    ]

    acceptable_onset_value_lst_right = list(np.arange(right_24, right_26, 0.01))
    acceptable_onset_value_lst_right = [
        float("%.2f" % elem) for elem in acceptable_onset_value_lst_right
    ]

    possible_onsets_left = df[df["Force_left[N]"].isin(acceptable_onset_value_lst_left)]
    possible_onsets_right = df[
        df["Force_right[N]"].isin(acceptable_onset_value_lst_right)
    ]

    onset_left = max(
        [x for x in list(possible_onsets_left.index) if x <= index_max_left]
    )
    offset_left = min(
        [x for x in list(possible_onsets_left.index) if x >= index_max_left]
    )
    ...
    onset_right = max(
        [x for x in list(possible_onsets_right.index) if x <= index_max_right]
    )
    offset_right = min(
        [x for x in list(possible_onsets_right.index) if x >= index_max_right]
    )
    ...
    df_right["Peak_difference[Peak-Force_right]"] = (
        max_right - df_right["Force_right[N]"]
    )
...
if uploaded_file is not None:
    ...
    df_left, df_right = output_calculations(
        uploaded_file, perc_margin, splits, viz=True, output=False, zoom=zoom
    )
